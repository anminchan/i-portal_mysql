...
<%@ include file="/includes/bbsContents.jsp" %>
	...
	<ul class="photo_list multi_type">
		<c:choose>          
				...
				<c:forEach items="${result }" var="data" varStatus="loop">
					...
					<c:if test="${!empty data.IMAGEFILENAME }">
						<li>
							...
							<div class="headline">
								<a href="${url }">
									<p class="cover_txt">${data.KNAME }</p>
						...
						<li>
							<span>
								<a href="${url }" class="txt">
									<p class="cover_txt"><text:splitText value="${data.KHTML }" tail="..." textSize="65" /></p>
									<span class="name">${data.CONTENTS1 }</span>
								</a>
							</span>
							<div class="headline">
								<a href="${url }">
