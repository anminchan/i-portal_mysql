...
<ul class="tab_menu">
	...
	<li class="on"><a href="#" id="btnCom">기업회원(${multiCount[1] > 0 ? multiCount[1] : 0 })</a></li> --%>
	<li><a href="#" id="btnEmp">내부직원(${multiCount[2] > 0 ? multiCount[2] : 0 })</a></li>
...
<form id="sform" name="sform" method="get" class="search_form">
	...
	<input type="hidden" name="kind" id="kind" value="${empty obj.kind ? 'C' : obj.kind }" />	   
...
<div class="table">
    <table class="tstyle_list">
        <caption>
            회원목록 - 번호, 아이디, 회사명, 대표자명, 담당장, 메일링여부, 가입일, 가입경로
        </caption>
        <colgroup>
            <col width="5%" />
			<col width="5%" />
        ...
        <c:choose>          
         <c:when test="${fn:length(rtnMemberList) > 0 }">
           ...
           <tr>
               ...
               <td>${memberList.CEONAME }</a></td>
               <td>${memberList.CHARGENAME }</a></td>
               <td>${memberList.MAILING == 'Y' ? '가입' : '미가입' }</a></td>
