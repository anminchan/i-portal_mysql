	...
	<article class="news_view">
		...
		<dl class="info">
			...
			<dd><c:if test="${!empty(rtnContent.KEYWORD1) }">${rtnContent.KEYWORD1}
				...
				<c:if test="${empty(rtnContent.KEYWORD1) || fn:length(rtnContent.KEYWORD1) <= 0 }"> 등록된 키워드가 없습니다. </c:if>
			</dd>
			<dt>공개여부</dt>
			...
			<c:if test="${rtnSetting.categoryYN == 'Y' }">
				<dt>카테고리</dt>
				<dd>${rtnSetting.noticeYN == 'Y' ? '' : 'colspan="3"' }>${rtnContent.CATEGORYNAME }</dd>
			</c:if>
		...
		<c:if test="${fn:length(rtnComment) > 0 }"> 
			<div class="reply_answers">
				<h2 class="reTitle">답변내용</h2>
				<p class="info"><strong>답변자</strong> : <c:out value="${rtnComment.USERNAME }" /> /
					<strong>답변일</strong> : <c:out value="${fn:substring(rtnComment.DMLTIME, 0, 4) }-${fn:substring(rtnComment.DMLTIME, 4, 6) }-${fn:substring(rtnComment.DMLTIME, 6, 8) }" />
				</p>
				<div>
					${rtnComment.KHTML }
				</div>
...
<c:if test="${rtnContent.APPEALUSERID == obj.myId }"> 
	<div class="btn_area">
		<button type="button" class="btn btn_type02" id="btnEdit">${rtnContent.PROCESS == 'R' ? '답변등록' : '답변수정'}</button>
	</div>
</c:if>
