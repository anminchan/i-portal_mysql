...
<spform:form id="searchForm" name="searchForm" method="get" action="${contextPath }/mgr/contentMgr" class="search_form">
    ...
    <input type="hidden" name="transCnt" value="${transCnt}">
	<fieldset>
		<legend>검색조건</legend>
		            ...
		            <tr ${data.NUM1 > 9000000000 ? 'style="background:#eee;"' : ''}>
		                <td>
		                    <c:if test="${data.NUM1 < 9000000000 }">
		                    	...
		                    	<input type="hidden" id="titleNm_${data.LINKID }" value="${data.KNAME }" />
		                    </c:if>
		                </td>
		                <td>
		                ...
		                <td class="txt_left">                   
		                   ...
		                   <c:if test="${data.SECRETTITLEYN  == 'Y'}"><i class="xi-lock-o"></i> <span class="hidden">비밀글</span></c:if>
		                </td>
...
$(function() {
    ...
    $('#btnAdd').click(function(){
        document.location.href="${contextPath}/mgr/contentMgr/form?menuId=${obj.menuId}&boardKind=NOTICE";
    });
