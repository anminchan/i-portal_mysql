...
<spform:form id="listForm" name="listForm" action="${contextPath }/mgr/contentMgr/actThumbnailBoard" method="POST">
		...
		<c:choose>          
			<c:when test="${!empty result && fn:length(result) > 0 }">
				<c:forEach items="${result }" var="data" varStatus="loop">
					<c:set var="url" value="${contextPath}/mgr/contentMgr/view?pageNum=${obj.pageNum }&rowCnt=${obj.rowCnt }&linkId=${data.LINKID }${link}"/>
					<li><input type="checkbox" id="linkId" name="linkId" class="listCheck" value="${data.LINKID }" bbsShareValue="${data.MENUID }§§${data.LINKID }§§${data.TITLEID }" title="선택" />
						<a href="${url }">
							...
							<span class="desc"><text:splitText value="${data.KHTML }" tail="..." textSize="55" /></span>
						</a>
					</li>
				 </c:forEach>
			</c:when>
...
$(function() {
    ...
    $('#btnTextStyle').click(function(){
        document.location.href="${contextPath}/mgr/contentMgr?menuId=${obj.menuId}&boardKind=THUMBNAIL&boardStyle=Text";
    });
    
    //텍스트 + 이미지 스타일
    $('#btnImageStyle').click(function(){
        document.location.href="${contextPath}/mgr/contentMgr?menuId=${obj.menuId}&boardKind=THUMBNAIL&boardStyle=Image";
    });
    
    // 갤러리 스타일
    $('#btnGalleryStyle').click(function(){
        document.location.href="${contextPath}/mgr/contentMgr?menuId=${obj.menuId}&boardKind=THUMBNAIL&boardStyle=Gallery";
    });
