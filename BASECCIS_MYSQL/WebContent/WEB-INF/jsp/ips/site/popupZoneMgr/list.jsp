...
<form id="listForm" name="listForm" action="${ctxMgr }/popupZoneMgr/delete" method="POST">
	...
	<div class="table">
		<table class="tstyle_list" >
			<caption>
				팝업존관리 목록 - 선택여부, 번호, 사이트명, 이미지, 제목, 게시시작일, 게시종료일, 순서, 사용여부
			</caption>		
			<colgroup>
				<col class="allChk"/>
				<col class="num" />
			...
			<c:choose>			
				<c:when test="${!empty result }">
					...
					<tr>
						<td><input type="checkbox" id="chkPopupZoneIds" name="chkPopupZoneIds" class="listCheck" value="${data.popupZoneId }" title="선택" /></td>
						<td>${data.no1 }</td>
						<td>${data.site_Name}</td>
						<td><a href="${ctxMgr }/popupZoneMgr/form?pageNum=${ obj.pageNum }&popupZoneId=${data.popupZoneId }${link}">
								<img alt="${data.imageFileName }" src="${contextPath}/fileDownloadThumbnails?fileGubun=common&menuId=popupZoneMgr&userFileName=${data.imageFileName }&systemFileName=${data.imageSFileName }" />
							</a>
						</td>
						<td class="txt_left"><a href="${ctxMgr }/popupZoneMgr/form?pageNum=${ obj.pageNum }&popupZoneId=${data.popupZoneId }${link}">${data.KName }</a></td>
						<td>
...
$(function() {
	...
	$('#btnSearch').click(function(){
		...
		$("#searchForm").attr('action', '${ctxMgr }/popupZoneMgr');
		$('#searchForm').submit();
	}); 
	...
	$('#btnExcel').click(function(){
		$("#searchForm").attr('action', '${ctxMgr }/popupZoneMgr/excelDown');
		$('#searchForm').submit();
	});
	
	$('#btnAdd').click(function(){
		document.location.href="${ctxMgr }/popupZoneMgr/form?${link}&pageNum=${ obj.pageNum }";
	});
	...
	$("#btnDelete").on("click", function() {
		...
		$nCnt = $("input:checkbox[name=chkPopupZoneIds]:checked").length;;
		
		if( $nCnt > 0 ){
			if(confirm("삭제하시겠습니까?")) {
				$("#listForm").attr('action', '${ctxMgr }/popupZoneMgr/delete');
				$("#listForm").submit();
			}
		}else{
