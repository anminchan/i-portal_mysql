	...
	<fieldset>
		...
		<div class="form_group">
			<div class="row">
				...
				<select name="siteIdSel" id="siteIdSel">
				</select>
			</div>
			<div class="row">
				<label for="schKName">배너명</label>
...
<form id="listForm" name="listForm" action="${ctxMgr }/bannerMgr/delete" method="POST">
	<input type="hidden" name="menuId" id="menuId" value="${param.menuId}">
	...
	<div class="table">
		<table class="tstyle_list">
			<caption>
				베너관리 목록 - 선택여부, 번호, 사이트명, 배너명, 순서, 사용여부
			</caption>
			<colgroup>
				<col class="allChk"/>
				<col class="num" />
			...
			<c:choose>			
				<c:when test="${!empty result }">
					...
					<tr>
						<td><input type="checkbox" id="chkBannerIds" name="chkBannerIds" class="listCheck" value="${data.bannerId }" title="선택" /></td>
						<td>${data.no1 }</td>
						<td>${data.site_Name }</td>
						<td><a href="${ctxMgr }/bannerMgr/form?pageNum=${ obj.pageNum }&bannerId=${data.bannerId }${link}">${data.KName }</a></td>
						<td>${data.sort }</td>
						<td>
...
$(function() {
	...
	$('#btnSearch').click(function(){
		...
		$("#searchForm").attr('action', '${ctxMgr }/bannerMgr');
		$('#searchForm').submit();
	}); 
	...
	$('#btnExcel').click(function(){
		$("#searchForm").attr('action', '${ctxMgr }/bannerMgr/excelDown');
		$('#searchForm').submit();
	});
	
	$('#btnAdd').click(function(){
		document.location.href="${ctxMgr }/bannerMgr/form?${link}&pageNum=${ obj.pageNum }";
	});
	...
	$("#btnDelete").on("click", function() {
		...
		$nCnt = $("input:checkbox[name=chkBannerIds]:checked").length;;
		
		if( $nCnt > 0 ){
			if(confirm("삭제하시겠습니까?")) {
				$("#listForm").attr('action', '${ctxMgr }/bannerMgr/delete');
				$("#listForm").submit();
			}
		}else{
