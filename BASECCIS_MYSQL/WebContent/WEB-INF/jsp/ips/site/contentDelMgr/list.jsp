...
<spform:form id="searchForm" name="searchForm" method="get" action="${contextPath }/mgr/contentDelMgr" class="search_form">
...
<spform:form id="listForm" name="listForm" action="${contextPath }/mgr/contentDelMgr/actDelBoard" method="POST">
	...
	<div class="float_wrap">
		...
		<div class="float_right">
			<button type="button" class="btn btn_basic" id="btnRestore">복구</button>
		</div>
	</div>
	<div class="table">
					...
					<c:forEach items="${result }" var="data" varStatus="loop">
						<tr>
							...
							<td class="txt_left">
							   ...
							   <a href="${contextPath}/mgr/contentDelMgr/view?pageNum=${obj.pageNum }&rowCnt=${obj.rowCnt }&linkId=${data.LINKID }&paramMenuId=${data.MENUID }${link}">${data.KNAME }</a>
...
$(function() {
	...
	$("#btnRestore").on("click", function() {
		...
		if( $nCnt > 0 ){
			if(confirm("복구하시겠습니까?")) {
				$("#listForm").attr('action', '${ctxMgr }/contentDelMgr/act');
				$("#listForm").submit();
			}
		}else{
			alert('복구 할 데이타를 선택하세요.');
		}
		return false;
	});
