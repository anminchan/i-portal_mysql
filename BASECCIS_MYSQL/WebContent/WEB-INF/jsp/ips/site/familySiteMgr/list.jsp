...
<form id="listForm" name="listForm" action="${ctxMgr }/familySiteMgr/delete" method="POST">
	<input type="hidden" name="menuId" id="menuId" value="${param.menuId}">
	...
	<div class="table">
		<table class="tstyle_list" >
			<caption>
				패밀리사이트 관리 - 번호, 사이트명, Family사이트명, 순서 사용여부 목록
			</caption>
			<colgroup>
				<col class="allChk"/>
				<col class="num" />
			...
			<c:choose>			
				<c:when test="${!empty result }">
					...
					<tr>
						<td><input type="checkbox" id="chkSiteLinkIds" name="chkSiteLinkIds" class="listCheck" value="${data.siteLinkId }" title="선택" /></td>
						<td>${data.no1 }</td>
						<td>${data.site_Name }</td>
						<td><a href="${ctxMgr }/familySiteMgr/form?pageNum=${ obj.pageNum }&siteLinkId=${data.siteLinkId }${link}">${data.KName }</a></td>
						<td>${data.sort }</td>
						<td>
...
$(function() {
	...
	$('#btnSearch').click(function(){
		...
		$("#searchForm").attr('action', '${ctxMgr }/familySiteMgr');
		$('#searchForm').submit();
	}); 
	...
	$('#btnExcel').click(function(){
		$("#searchForm").attr('action', '${ctxMgr }/familySiteMgr/excelDown');
		$('#searchForm').submit();
	});
	
	$('#btnAdd').click(function(){	
		document.location.href="${ctxMgr }/familySiteMgr/form?${link}&pageNum=${ obj.pageNum }";
	});
	...
	$("#btnDelete").on("click", function() {
		...
		$nCnt = $("input:checkbox[name=chkSiteLinkIds]:checked").length;;
		
		if( $nCnt > 0 ){
			if(confirm("삭제하시겠습니까?")) {
				$("#listForm").attr('action', '${ctxMgr }/familySiteMgr/delete');
				$("#listForm").submit();
			}
		}else{
