...
<h2 class="depth2_title">관리자 답변현황</h2>
<div class="table">
	<table class="tstyle_list">
		<caption>
			답변현황
		...
		<thead>
			<tr>
				<th scope="col">사이트</th>
				<th scope="col">메뉴</th>
				<th scope="col">미답변건수</th>
			</tr>
		</thead>
		<tbody>
		<c:choose>	
			<c:when test="${!empty resultAnswer }">
				<c:set var="nTotal" value="0" />
				<c:forEach items="${resultAnswer }" var="data" varStatus="loop">
					<tr>
						<td>${data.SITENAME }</td>
						<td>${data.MENUNAME }</td>
						<td>${data.CNT }</td>
...
<h2 class="depth2_title">CMS 공지</h2>
<div class="table">
	<table class="tstyle_list">
		<caption>
			CMS 공지
		...
		<tbody>
		<c:choose>	
			<c:when test="${!empty resultNotice }">
				<c:set var="nTotal" value="0" />
				<c:forEach items="${resultNotice }" var="data" varStatus="loop">
					<tr>
...
$(function() {	
	...
	$('#btnSearch').click(function(){
		...
		$("#searchForm").attr('action', '${ctxMgr }/main');
		$('#searchForm').submit();
	});
