...
<form action="${contextPath}/mps/memberSearch/passwordMailCorfirm" id="memberPasswordSearchForm">
...
<div class="member_box">
 	<p class="member_info"><strong class="point02">비밀번호가 생각나지 않으세요?</strong>
		공공I-Pin인증 또는 실명인증(휴대폰인증)을 통해 본인 확인 후 회원가입 시 등록하신 이메일로 임시비밀번호를 안내해드립니다.</p>
	<input type="hidden" id="menuId" name="menuId" value="${param.menuId}"/>
...
function fnSetCertifyCheck(KName, dkey, birthDate, certification, gender){
	$.ajax({
 		...
 		data: {'dkey' : dkey, 'kind' : 'P', 'schType' : '2'}, //schType : 2(Password찾기)
 		async: false,
 		cache: false,
 		success:function(data, textStatus, jqXHR) {
			if(data != ""){
				...
				$("#email").val(data.receiveMail);
				$("#memberPasswordSearchForm").submit();
			}else{
				alert("입력하신 정보와 일치하는 정보가 없습니다.\n 정확한 정보를 다시 입력해 주시기 바랍니다.");
			}
 		}
 	});
}
...
function fnSetCertifyFail(message){
	alert("휴대폰 인증 실패::Code::"+errorCode+"::"+message);
}
