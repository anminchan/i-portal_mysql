			...
			<c:when test="${!empty result && fn:length(result) > 0 }">
				<c:forEach items="${result }" var="data" varStatus="loop">
					...
					<a href="${url }" class="thumb">
						...
						<img alt="<c:out value='${!empty data.ALTINFO ? data.ALTINFO : data.KNAME }'/>" src="${contextPath}/fileDownload?titleId=${data.TITLEID }" onerror="this.src='${contextPath}/resources/images/ips/bbs/noimage.gif'"/>
						</c:if>
						<c:if test="${empty data.IMAGEFILENAME }">
							<img src="${contextPath}/resources/images/ips/bbs/noimage.gif" alt="KHIDI"/>
						</c:if>
					</a>
						<div class="headline">
							...
							<a href="${url }" class="desc"><text:splitText value="${data.KHTML }" tail="..." textSize="95" /></a>
