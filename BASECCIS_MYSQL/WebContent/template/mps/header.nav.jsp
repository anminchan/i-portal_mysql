...
<div class="logo_wrap wrap_con">
	<c:choose>
		<c:when test="${not empty rtnDoodlesList}">
			<h1><a href="${contextPath}/mps"><img src="${contextPath}/fileDownload?fileGubun=common&menuId=doodlesMgr&userFileName=${rtnDoodlesList[0].imageFileName}&systemFileName=${rtnDoodlesList[0].imageSFileName}" alt="plani_${rtnDoodlesList[0].KName }"></a></h1>
		</c:when>
		<c:otherwise>
			<h1 class="logo"><a href="${contextPath}/mps">plani</a></h1>
		</c:otherwise>
	</c:choose>
	<nav>
		<ul id="gnavigation">
		...
		<c:forEach items="${rtnUserMenu }" var="topMenu">
			<c:if test="${topMenu.DEPTH == 1 and topMenu.DISPLAYYN eq 'Y' }">
				...
				<li id="topNaviCom_${topMenu.MENUID}" class="menu0${menuCnt }">
